...
const UserView = () => {
	...
	useEffect(() => {
		...
		const readContent = async () => {
			const contentData = await userBoardsFetch.readContent(contentCode, "view");
	...
	useEffect(() => {
		if(contentJson === null){
			...
			const deleteContent = async () => {
				...
				const deleteResult = await userBoardsFetch.deleteContent(sendData);
			...
			const voteContent = async (voteType) => {
				...
				if(contentCode !== null){
					const voteResult = await userBoardsFetch.voteContent(voteType, sendData);
